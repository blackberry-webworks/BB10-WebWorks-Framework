...
eventHandlers =  {
    ...
    init : function (webview) {
        ...
        _webview.addEventListener("DocumentLoadFinished", function () {
            if (webview.currentLocation === NETWORK_ERROR_PAGE && _pendingRequest) {
                _webview.executeJavascript("window.redirectUrl = '" + _pendingRequest.url + "';");
    ...
    onNetworkError : function (request) {
        ...
        if (requestObj.type === NETWORK_ERROR) {
            // Is this a request to load a remote page?
            if (requestObj.url === _webview.currentLocation && _utils.isRemote(requestObj.url)) {
    ...
    networkCacheResponse : function (cached) {
        if (_pendingRequest) {
            ...
            if (JSON.parse(cached)) {
                ...
                _webview.setURL(NETWORK_ERROR_PAGE);
            }
        }
    },
