...
function runHandler(actionId, item) {
    if (_customHandlers[actionId]) {
        _customHandlers[actionId](actionId);
    } else if (_handlers[actionId]) {
        _handlers[actionId](actionId);
    } else if (_menuServiceHandlers[actionId]) {
        _menuServiceHandlers[actionId](item);
    }
}
...
function menuServiceHandler(item) {
    ...
    if (item.children) {
        request = {
            action: item.children.items[0].invoke.action,
            type: item.children.items[0].invoke.mime,
            file_transfer_mode: item.children.items[0].invoke.file_transfer_mode,
            //uri: item.children.items[0].invoke.uri,
            data: item.children.items[0].invoke.data
            //target_type_mask: _application.invocation.TARGET_TYPE_MASK_APPLICATION | _application.invocation.TARGET_TYPE_MASK_CARD |
            //  _application.invocation.TARGET_TYPE_MASK_SERVICE,
        };
        ...
        for (i = 0; i < item.children.items.length; i++) {
            target = {
                key: item.children.items[i].invoke.target,
                icon: item.children.items[i].icon,
                label: item.children.items[i].label,
                type: item.children.items[i].invoke.type,
            };
            results[0].targets.push(target);
        }
        debugger;
        showInvocationList(item.label, request, results);
    } else {
        debugger;
        request = {
            action: item.invoke.action,
            target: item.invoke.target,
            file_transfer_mode: item.invoke.file_transfer_mode,
            //data  : window.btoa(item.invoke.data)
            data  : item.invoke.data
        };

        if (item.invoke.uri) {
            request.uri = item.invoke.uri;
        }
        debugger;
        window.qnx.webplatform.getApplication().invocation.invoke(request, function () {
            console.log("Menu Service Invoke");
        });
    }
}
...
_menuServiceHandlers = {
    'menuService'    : menuServiceHandler
};

menuActions = {
    handlers: _handlers,
    menuServiceHandlers: _menuServiceHandlers,
