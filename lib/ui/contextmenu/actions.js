...
function menuServiceHandler(item) {
    ...
    if (item.children) {
        ...
        request = {
            ...
            //type  : item.invoke.type,
            uri   : item.invoke.uri,
            //data  : window.btoa(item.invoke.uri)
        };
        if (item.invoke.mime) {
            request.type = item.invoke.mime;
        }
        if (item.invoke.data) {
            request.data = item.invoke.data;
        } else if (item.invoke.uri) {
            request.data = window.btoa(item.invoke.uri);
        }
