...
var _overlayWebView = require('./../../../lib/overlayWebView'),
    ...
    _dialog = require('./../../../lib/ui/dialog/index.js'),
    _menuItems,
    _currentContext,
...
// Generate the list of menu items for menu server
function generateInvocationListForMenuService(item) {
    _overlayWebView.executeJavascript("window.showTargetsForMenuService(" + JSON.stringify(item) + ")");
}
...
function _menuService(item) {
    var request = {};
    // Has child items
    if (item.children) {
        generateInvocationListForMenuService(item);
    } else {
        request = {
            action: item.invoke.action,
            target: item.invoke.target,
            type  : item.invoke.mime,
