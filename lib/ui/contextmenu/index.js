...
var contextmenu,
    LIB_PATH = "./../../../lib",
    _webview = require(LIB_PATH + '/webview'),
...
function addItemsForMenuService(items, menuServiceResponse) {
    var i,
        item,
        fakeData = {
            "items": [
                {
                    ...
                    "label": "Share",
                    "invoke": {"type": "CARD", "target": "sys.NFCViewer", "action": "bb.action.SHARE", "mime": "test/*", "data": "Fake data", "perimeter": "personal"}
                },
                {
                    ...
                    "children": {
                        "items": [
                            {
                                "icon": "/apps/sys.catalyst.testRel_ys_catalyst38fc547a/public/native/CatalystAppIcon.png",
                                "label": "Plans",
                                "invoke": {"type": "APPLICATION", "target": "sys.catalyst", "action": "bb.action.OPEN", "mime": "test/*", "data": "Fake data", "perimeter": "personal"}
                            },
                            {
                                "icon": "/apps/sys.appworld.testRel_ys_appworld7d1b5812/public/native/storeicon.png",
                                "label": "App World",
                                "invoke": {"type": "APPLICATION", "target": "sys.appworld.push", "action": "bb.action.OPEN", "mime": "test/*", "data": "Fake data", "perimeter": "personal"}
                            }
                        ]
                    }
                }
    ...
    //FIXME: Use fake data now
    menuServiceResponse = fakeData.items; 

    for (i in menuServiceResponse) {
        item = {
            'label'   : menuServiceResponse[i].label,
            'actionId': 'menuService', 
            'imageUrl': "file://" + menuServiceResponse[i].icon,
            'invoke'  : menuServiceResponse[i].invoke
        };

        if (menuServiceResponse[i].children) {
            item.children = menuServiceResponse[i].children;
        }
...
function getMenuFromMenuService() {
    var menuService = require("./../../../lib/menu.service"),
        request = {
...
function init() {
    ...
    _overlayWebView.onContextMenuRequestEvent = function (value) {
        ...
        if (contextMenuEnabled) {
            // Add items from menu service
            addItemsForMenuService(menuItems, menuServiceResponse);
