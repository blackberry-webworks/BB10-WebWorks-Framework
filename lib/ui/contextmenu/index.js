...
function addMenuServiceItems(items, context) {
    var menuService = require("./../../../lib/menuService"),
        request = {
            file_transfer_mode: _invocation.FILE_TRANSFER_COPY_RO,
            target_type_mask: _invocation.TARGET_TYPE_MASK_APPLICATION | _invocation.TARGET_TYPE_MASK_CARD | _invocation.TARGET_TYPE_MASK_SERVICE
        };

    // mime type: image/*
    if (context && context.isImage) {
        request.mime = "image/*";
        request.data = context.src;
        request.uri = context.src;
    } else if (context && context.text) { // mime type: text/*
        request.mime = "text/plain";
        if (context.url) {
            request.uri = context.url;
            if (!(/^tel/.test(context.url))) {
                request.data = context.url;
            }
        } else {
    ...
    menuService.getMenuItems(request, function (response, error) {
        var responseItems = JSON.parse(response).items,
            item,
            last2Items,
            args,
            i;
        ...
        for (i = 0; i < responseItems.length; i++) {
            if (!(/^local|^file/.test(context.url) && responseItems[i].label === 'Share')) {
                item = {
                    'label'   : responseItems[i].label,
                    'actionId': 'menuService', 
                    'imageUrl': "file://" + responseItems[i].icon,
                };

                if (responseItems[i].children) {
                    item.children = responseItems[i].children;
                } else {
                    item.invoke = responseItems[i].invoke;
                    if (!item.invoke.uri) {
                        item.invoke.uri = request.uri;
                    }
                }

                // Currently the delete item is in the last two items
                last2Items = items.slice(-2);
                if (last2Items && last2Items[0].isDelete) {
                    items.splice(-2, 0, item);
                } else if (last2Items && last2Items[1].isDelete) {
                    items.splice(-1, 0, item);
                }
            }
        }
        debugger;
        args = JSON.stringify({'menuItems': items, '_currentContext': context});
...
function init() {
    ...
    _overlayWebView.onContextMenuRequestEvent = function (value) {
        ...
        generateMenuItems(menu.menuItems, function (items) {
            if (contextMenuEnabled) {
                ...
                addMenuServiceItems(items, _currentContext);
            }
        });
    ...
    _controller.publishRemoteFunction('executeMenuAction', function (args, callback) {
        var actionId = args[0],
            option = args[1];
        if (actionId) {
            _actions.runHandler(actionId, option);
        }
    });
