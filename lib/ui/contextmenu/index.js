...
function generateMenuItems(menuItems) {
    ...
    for (i = 0; i < menuItems.length; i++) {
        ...
        case 'Delete':
            break;
        case 'Cancel':
            items.push({'label': 'Dismiss Selection', 'actionId': 'Cancel', 'imageUrl': 'assets/Browser_Cancel_Selection.png', 'isDelete': true});
            hasCancel = true;
            break;
        case 'Cut':
    ...
    if (!hasCancel) {
        items.push({'label': 'Cancel', 'actionId': 'Cancel', 'imageUrl': 'assets/Browser_Cancel_Selection.png', 'isDelete': true});
    }
...
function addCustomItemsForContext(items, context) {
    ...
    if (customContextItems) {
        if (lastItemIsDelete) {
            lastItem = items.pop();
            items = items.concat(customContextItems, lastItem);
        } else {
            items = items.concat(customContextItems);
        }
    }
}
