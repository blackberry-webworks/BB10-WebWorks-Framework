...
module.exports = {
    get: function (request, succ, fail) {
        var webworksJsHash,
            ...
            featureIds,
            xhr,
            errorMsg = "The webworks.js javascript library being used is incompatible with the WebWorks runtime." + 
                    "Please upgrade your webworks.js file.";
        
        //Extract webworksVersion from client webworks.js
        if (request.params["ext"]) {
            webworksJsHash = request.params["ext"].split("=")[1];
        }
        
        //read framework webworks.js version
        xhr = new XMLHttpRequest();
        xhr.open("GET", "webworks-version", false);
        xhr.send();
        frameworkHash = xhr.responseText;
