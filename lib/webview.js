...
webview =
    ...
    create: function (config, ready) {
        _webviewObj = window.qnx.webplatform.createWebView({processType: OUT_OF_PROCESS, defaultHandlers: ['onChooseFile']}, function () {
            ...
            /* Check if we should allow the networking and JavaScript engines to be available 
             * when the app is running in the background 
             */
            var shouldRunWhenBackgrounded = false;
            if (config.permissions && config.permissions.indexOf("run_when_backgrounded") >= 0) {
                shouldRunWhenBackgrounded = true;
            } else if (config.accessList) {
                /* Check if the blackberry.push feature is being used */
                var features = config.accessList[0].features;

                for (var i = 0; i < features.length; i++) {
                    if (features[i].id === "blackberry.push") {
                        shouldRunWhenBackgrounded = true;
                        break;
                    }
                }
            ...
            if (shouldRunWhenBackgrounded) {
                _webviewObj.autoDeferNetworkingAndJavaScript = false;
            } else {
                _webviewObj.autoDeferNetworkingAndJavaScript = true;
            }
