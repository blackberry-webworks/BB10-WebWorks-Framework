...
function registerInvocationEvents() {
    if(window.chrome && window.chrome.internal) {
        //Remove row below when invocation become attached to chrome.internal, also remove invocation.js along with requiring it.
        chrome.internal.invocation = invocation || {};
        chrome.internal.invocation.onInvoked = function (onInvokedInfo) {
            onInvokedInfo = JSON.parse(onInvokedInfo);
            if(onInvokedInfo.data) {
                onInvokedInfo.data = atob(onInvokedInfo.data);
            }

            if (typeof onInvoked === 'function') {
                onInvoked(onInvokedInfo);
            }
        };
    }
}
