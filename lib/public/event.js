    ...
    trigger: function (name, args) {
        ...
        if (_handlers.hasOwnProperty(name)) {
            if (args && args !== "undefined") {
                ...
                if (parsedArgs.fieldsToBlob) {
                    for (fieldName in parsedArgs.fieldsToBlob) {
                        if (parsedArgs.fieldsToBlob.hasOwnProperty(fieldName)) {
                            blobBuilderObj = new window.WebKitBlobBuilder();
                            parsedArgs[fieldName] = convertDataToBinary(parsedArgs[fieldName], parsedArgs.fieldsToBlob[fieldName]);
                            blobBuilderObj.append(parsedArgs[fieldName]);
                            parsedArgs[fieldName] = blobBuilderObj.getBlob("arraybuffer");
                        }
                    }

                    delete parsedArgs.fieldsToBlob;
                }
            }
            //Call the handlers
