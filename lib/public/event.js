...
function convertDataToBinary(data, dataEncoding) {
    var rawData,
        uint8Array,
        i;
...
module.exports = {
    ...
    trigger: function (name, args, argsToBlob) {
        var parsedArgs,
            fieldName,
            blobBuilderObj;
        ...
        if (_handlers.hasOwnProperty(name)) {
            if (args && args !== "undefined") {
                ...
                // Checking if there are fields that need to become Blob.
                if (argsToBlob && argsToBlob !== "undefined") {
                    argsToBlob = JSON.parse(argsToBlob);

                    for (fieldName in argsToBlob) {
                        if (argsToBlob.hasOwnProperty(fieldName)) {
                            blobBuilderObj = new window.WebKitBlobBuilder();
                            parsedArgs[fieldName] = convertDataToBinary(parsedArgs[fieldName], argsToBlob[fieldName]);
                            blobBuilderObj.append(parsedArgs[fieldName]);
                            parsedArgs[fieldName] = blobBuilderObj.getBlob("arraybuffer");
                        }
                    }
                }
            }
